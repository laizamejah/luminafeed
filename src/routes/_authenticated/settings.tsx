...
function PreferenceDetail({ me, fields }: { me: NonNullable<ReturnType<typeof useCurrentProfile>["data"]>; fields: { key: BooleanProfileKey; label: string; description: string }[] }) {
  ...
  async function update(key: BooleanProfileKey, value: boolean) {
    const changes = key === "show_metrics_publicly" ? { show_metrics_publicly: value }
      : key === "hide_public_counts" ? { hide_public_counts: value }
      : key === "message_notifications" ? { message_notifications: value }
      : { hide_reels: value };
...
function PeopleList({ people, loading, empty, action }: { people: { id: string; username: string; display_name: string | null; avatar_url: string | null }[]; loading: boolean; empty: string; action?: (person: { id: string; username: string; display_name: string | null; avatar_url: string | null }) => ReactNode }) { if (loading) return <div className="p-5 text-sm text-muted-foreground">Loading…</div>; if (!people.length) return <div className="p-8 text-center text-sm text-muted-foreground">{empty}</div>; return <div className="divide-y divide-border">{people.map((person) => <div key={person.id} className="flex items-center gap-3 p-4"><AvatarImage path={person.avatar_url} name={person.display_name ?? person.username} size={46} /><div className="min-w-0 flex-1"><p className="truncate font-medium">{person.display_name || person.username}</p><p className="truncate text-sm text-muted-foreground">@{person.username}</p></div>{action?.(person)}</div>)}</div>; }
...
function InfoDetail({ title, body, icon: Icon, children }: { title: string; body: string; icon: Icon; children?: ReactNode }) { return <div className="p-7 text-center"><span className="mx-auto grid h-16 w-16 place-items-center rounded-full bg-muted"><Icon className="h-8 w-8" /></span><h2 className="mt-5 font-sans text-xl font-bold">{title}</h2><p className="mx-auto mt-2 max-w-md text-sm leading-6 text-muted-foreground">{body}</p>{children}</div>; }
